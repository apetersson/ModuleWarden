...
or the LEONARDO_USERNAME== / LEONARDO_PASSWORD env vars. Nothing is hardcoded.
...
def load_credentials() -> tuple[str, str, str]:
    user = os.environ.get("LEONARDO_USERNAME==")
    ...
    if not (user and pw):
        ...
        for path in (
            ...
            if path.is_file():
                ...
                user = user or kv.get("LEONARDO_USERNAME==") or kv.get("USERNAME")
                pw = pw or kv.get("LEONARDO_PASSWORD") or kv.get("PASSWORD")
                login = kv.get("LEONARDO_LOGIN", login)
                break
    if not (user and pw):
        sys.exit("No credentials. Set LEONARDO_USERNAME==/LEONARDO_PASSWORD or use a "
