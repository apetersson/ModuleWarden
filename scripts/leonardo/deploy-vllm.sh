...
#   - LEONARDO_USER and LEONARDO_PASSWORD set (or in ~/.leonardo-env)
#
# Usage:
...
scp "${SLURM_SCRIPT}" "${LEONARDO_USER}@${LEONARDO_LOGIN}:~/mw-slurm-vllm.sh"

# ── Submit job ──────────────────────────────────────────────
echo "Submitting vLLM job..."
JOB_ID=$(ssh "${LEONARDO_USER}@${LEONARDO_LOGIN}" \
...
for i in $(seq 1 60); do
    STATE=$(ssh "${LEONARDO_USER}@${LEONARDO_LOGIN}" \
    ...
    if [ "${STATE}" = "UNKNOWN" ] || [ "${STATE}" = "FAILED" ] || [ "${STATE}" = "CANCELLED" ]; then
        ...
        echo "Check logs: ssh ${LEONARDO_USER}@${LEONARDO_LOGIN} 'cat slurm-vllm-${JOB_ID}.err'"
        exit 1
    fi
...
# ── Get compute node ────────────────────────────────────────
NODE=$(ssh "${LEONARDO_USER}@${LEONARDO_LOGIN}" \
...
echo ""
echo "Monitor logs:"
echo "  ssh ${LEONARDO_USER}@${LEONARDO_LOGIN} 'tail -f slurm-vllm-${JOB_ID}.out'"
echo ""
echo "Cancel job:"
echo "  ssh ${LEONARDO_USER}@${LEONARDO_LOGIN} 'scancel ${JOB_ID}'"
