...
- [ ] #7 The architecture defines cold start as an initial all-lockfiles audit campaign, not a normal diff, and explicitly rejects admin baseline allow shortcuts for v1.
- [ ] #8 Cold-start verdict semantics are conservative: allow requires clean provenance, behavior, install traces, and heuristic evidence; uncertainty quarantines.
- [ ] #9 The trust model treats the model endpoint operator as trusted infrastructure and documents logging controls plus the prompt-secrecy degradation when logging is unverified.
- [ ] #10 Recorded-open egress means public internet only, with host, internal networks, link-local metadata services, and direct backing-service access blocked.
