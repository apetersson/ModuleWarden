...
Optional real-classifier arm, the honest "real number" the text corpus could not give. Using the task-36 `tarball-features.jsonl` (3,778 vulnerable + 2,809 benign), train an XGBoost / RandomForest on the static feature set (install-scripts, entropy, file_count, suspicious requires, obfuscation; optionally merge GuardDog rule flags). Report AUROC on a 20% stratified hold-out. The benchmark (arXiv:2603.27549, figshare 13,708-package set) shows GuardDog-class features reach ~93% F1, but that is malware-vs-benign typosquat detection, a different problem. MEASURED RESULT on THIS corpus (recorded in decision-10 and CLASSIFIER-FLOOR-FINDINGS.md): the cold-package static classifier floors at AUROC 0.54, because the GHSA pairs share near-identical absolute capability inventories. The signal is in the version DELTA, not the cold package. The honest output of this arm is that floor and the reason for it, not a borrowed 0.90, with the deterministic delta-gate still the verdict authority.
